...
const db = require("@data/noprefix");
...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    if (args[0].toLowerCase() === "list") {
      ...
      const createEmbed = () =>
        new client.embed()
          .t(`${client.user.username} NoPrefix List`)
          .img(client.config.links.icon)
          .d(pages[page])
          .setFooter({
    ...
    const userEmbed = new client.embed()
    ...
    collector.on("collect", async (interaction) => {
      ...
      if (interaction.customId === "remove-noprefix") {
        ...
        return interaction.update({
          embeds: [
            new client.embed().d(`✅ Removed **NoPrefix** from ${user}.`),
          ],
          components: [],
        });
      }
        ...
        tierCollector.on("collect", async (tierInteraction) => {
          const expiresAt =
            tierInteraction.customId === "diamond"
              ? null
              : Date.now() +
                { bronze: 86400000, silver: 604800000, gold: 2592000000 }[
                  tierInteraction.customId
                ];
          await db.updateOne(
            { userId: user.id },
            { userId: user.id, noprefix: true, expiresAt },
            { upsert: true }
          );
