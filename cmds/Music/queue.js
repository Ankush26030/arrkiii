...
const previousTracks = [...player.queue.previous]
  ...
  .map(
    (t, i) =>
      `-# ${client.emoji.dot} ${i + 1}: ${client.cleanTitle(t.title, t.author)} | [${t.requester.displayName}](${client.url(t.requester.id)}) ${
        t.isStream ? "◉ LIVE" : convertTime(t.length)
      }`
  );

    const currentTrack = ` **${client.emoji.dot} ${previousTracks.length + 1}: ${client.cleanTitle(player.queue.current.title, player.queue.current.author)} | [${player.queue.current.requester.displayName}](${client.url(player.queue.current.requester.id)}) ${
      player.queue.current.isStream
        ? "LIVE"
        : convertTime(player.queue.current.length)
    } ${client.emoji.tick}**`;

    const upcomingTracks = player.queue.map(
      (t, i) =>
        `-# ${client.emoji.dot} ${i + previousTracks.length + 2}: ${client.cleanTitle(t.title, t.author)} | [${t.requester.displayName}](${client.url(t.requester.id)}) ${convertTime(t.length)}`,
    );

    const fullQueue = [...previousTracks, currentTrack, ...upcomingTracks];
    const chunks = _.chunk(fullQueue, 10);
